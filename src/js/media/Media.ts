import { classMixin, linkify, mergeData } from '../core/Util'
...
class Media {
  ...
  loadMedia() {
    ...
    if (!this._state.loaded) {
      try {
        ...
        trace(
          'Error loading media for ', this._media,
        )
        trace(e)
      }
    }
  ...
  loadErrorDisplay(message) {
    ...
    this._el.content_item = DOM.create(
      'div', 'tl-media-item tl-media-loaderror', this._el.content,
    )
  ...
  showMeta(
    ...
    if (this.data.credit && this.data.credit != '') {
      this._el.credit = DOM.create(
        'div', 'tl-credit', this._el.content_container,
      )
    ...
    if (this.data.caption && this.data.caption != '') {
      this._el.caption = DOM.create(
        'div', 'tl-caption', this._el.content_container,
      )
  ...
  _initLayout() {
    ...
    this.message = new Message(
      this._el.container, this.options, this.getLanguage(),
    )
    // this.message.addTo(this._el.container);

    // Create Layout
    this._el.content_container = DOM.create(
      'div', 'tl-media-content-container', this._el.container,
    )

    // Link
    if (this.data.link && this.data.link != '') {
      this._el.link = DOM.create(
        'a', 'tl-media-link', this._el.content_container,
      )
      ...
      if (this._el.link.target == '_blank') {
        this._el.link.setAttribute(
          'rel', 'noopener',
        )
      }

      this._el.content = DOM.create(
        'div', 'tl-media-content', this._el.link,
      )
    }
    else {
      this._el.content = DOM.create(
        'div', 'tl-media-content', this._el.content_container,
      )
    }
  }
