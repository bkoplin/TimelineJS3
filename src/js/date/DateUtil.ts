...
import { BigDate, TLDate } from './TLDate'

export function sortByDate(
  array, prop_name,
) { // only for use with slide data objects
...
export function parseTime(time_str) {
  ...
  if (match) {
    period = trim(match[0])
    time_str = trim(time_str.substring(
      0, time_str.lastIndexOf(period),
    ))
  }
  ...
  if (parts.length > 1) {
    ...
    if (isNaN(parsed.minute)) {
      throw new TLError(
        'invalid_minute_err', minute_part,
      )
    }
  }
  ...
  if (parts.length > 2) {
    ...
    if (parts.length == 2) {
      ...
      if (isNaN(frac_secs)) {
        throw new TLError(
          'invalid_fractional_err', parts[1],
        )
      }
