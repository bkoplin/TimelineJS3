...
const phantomjs = ua ? ua.includes('phantom') : false
...
export const gecko = ua.includes('gecko') && !webkit && !opera && !ie
export const firefox = ua.includes('gecko') && !webkit && !opera && !ie
export const chrome = ua.includes('chrome')
export const edge = ua.includes('edge/')
...
if (!retina && window && 'matchMedia' in window) {
  const media_matches = window.matchMedia('(min-resolution:144dpi)')
