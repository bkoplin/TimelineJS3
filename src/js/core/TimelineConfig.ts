  ...
  'afterSanitizeAttributes', (node) => {
    if (node.nodeName == 'A' && 'href' in node) {
      ...
      if (!rel) {
        ...
        if (!rel.value.includes('noopener')) {
          node.setAttribute(
            'rel', `noopener ${rel.value}`,
          )
        }
      }
    }
...
function _process_fields(
  slide, callback, fieldmap,
) {
  Object.keys(fieldmap).forEach((k) => {
    const to_sanitize = (k == 'slide') ? slide : slide[k]
    if (to_sanitize) {
...
export class TimelineConfig {
  ...
  addEra(data) {
    ...
    if (typeof (data.start_date) == 'undefined') {
      throw new TLError(
        'missing_start_date_err', event_id,
      )
    }
  ...
  /**
     ...
     * when assigned here.
     */
  _assignID(slide: TimelineEvent) {
    let slide_id = slide?.unique_id ?? ''
    if ((slide?.unique_id ?? '').length === 0) {
      // give it an ID if it doesn't have one
      slide_id = (slide.text) ? kebabCase(slide.text.headline) : null
    }
    // make sure it's unique and add it.
    slide.unique_id = ensureUniqueKey(
      this.event_dict, slide_id,
    )
  ...
  /**
     ...
     * from the title ID.
     */
  _makeUniqueIdentifiers(
    title_id: TimelineEvent['unique_id'], array: TimelineEvent[],
  ) {
    const used = [title_id]
  ...
  _ensureValidScale(events: TimelineEvent[]) {
    if (!this.scale) {
      ...
      for (let i = 0; i < events.length; i++) {
        ...
        if (events[i].start_date && typeof (events[i].start_date.year) != 'undefined') {
          const d = new BigDate(events[i].start_date)
          const year = d.data.date_obj.year
  ...
  _processDates(slide_or_era) {
    ...
    if (!(slide_or_era.start_date instanceof dateCls)) {
      ...
      if (typeof (slide_or_era.end_date) != 'undefined' && !(slide_or_era.end_date instanceof dateCls)) {
        const end_date = slide_or_era.end_date
        let equal = true
        for (const property in start_date)
  ...
  getEarliestDate() {
    ...
    const date = this.events[0].start_date
  ...
  _tidyFields(slide: TimelineEvent|InstanceType<typeof TimeEra>) {
    ...
    _process_fields(
      slide, _tl_sanitize, SANITIZE_FIELDS,
    )
    // handle media.url separately
    _process_fields(
      slide, stripMarkup, STRIP_MARKUP_FIELDS,
    )
  }
}
