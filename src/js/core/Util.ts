...
import { attempt, isError, zipObject } from 'lodash'
import type { IterableElement } from 'type-fest'
import type { TimelineData } from './../timeline/Timeline.d'
...
export function hexToRgb(hex: Parameters<typeof chroma>[0]) {
  // Expand shorthand form (e.g. "03F") to full form (e.g. "0033FF")
  const isValid = chroma.valid(hex)
...
export function rgbToHex(rgb: { r: number; g: number; b: number } | string) {
  if (typeof rgb === 'string') {
    const isValid = chroma.valid(rgb)
    if (!isValid)
      return null
    else return chroma(rgb).hex()
  }
  const { r, g, b } = rgb
  return chroma(
    r, g, b, 'rgb',
...
export function ensureUniqueKey(
  obj: Record<string | IterableElement<TimelineData['events']>['unique_id'], IterableElement<TimelineData['events']>>, candidate?: string,
) {
  if (!candidate)
  ...
  for (let counter = similar_ids.length; similar_ids.includes(candidate); counter++)
...
export function findNextGreater(
  list, current, default_value,
) {
...
export function findNextLesser(
  list, current, default_value,
) {
  // given a sorted list and a current value which *might* be in the list,
...
export function linkify(
  ...
  const make_link = function (
    url, link_text, prefix,
  ) {
    if (!prefix)
      prefix = ''
    ...
    if (link_text && link_text.length > MAX_LINK_TEXT_LENGTH) {
      link_text = `${link_text.substring(
0, MAX_LINK_TEXT_LENGTH,
)}\u2026`
    } // unicode ellipsis
  ...
  return text
    .replace(
      urlPattern, (
        match, url_sans_protocol, offset, string,
      ) => {
      ...
      // we need to handle risk of matching URLs in legit hrefs
        ...
        return make_link(
          match, url_sans_protocol,
        )
      },
    )
    .replace(
      pseudoUrlPattern, (
        match, beforePseudo, pseudoUrl, offset, string,
      ) => {
        return make_link(
`http://${pseudoUrl}`, pseudoUrl, beforePseudo,
        )
      },
    )
    .replace(
      emailAddressPattern, (
        match, email, offset, string,
      ) => {
        return make_link(
`mailto:${email}`, email,
        )
...
export function transformMediaURL(url) {
  return url.replace(
    /(.*)www.dropbox.com\/(.*)/, '$1dl.dropboxusercontent.com/$2',
  )
}
