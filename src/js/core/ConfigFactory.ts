...
import { isEmptyObject, mergeData, trace, trim } from './Util'
...
function clean_integer(s) {
  ...
  } // doesn't handle '.' as comma separator, but how to distinguish that from decimal separator?
}
...
export function parseGoogleSpreadsheetURL(url) {
  ...
  const key_pat = /\bkey=([-_A-Za-z0-9]+)&?/i
  const url_pat = /docs.google.com\/spreadsheets(.*?)\/d\// // fixing issue of URLs with u/0/d
  ...
  if (url.match(key_pat)) {
    ...
    const pos = url.search(url_pat) + url.match(url_pat)[0].length
    const tail = url.substr(pos)
...
function extractEventFromCSVObject(orig_row) {
  ...
  if (Object.keys(row).includes('Start Date') || Object.keys(row).includes('End Date')) {
    ...
    if (row.Time) {
      if (d.start_date) {
        mergeData(
          d.start_date, parseTime(row.Time),
        )
      }

      else { throw new TLError('invalid_start_time_without_date') }
    }

    if (row['End Time']) {
      if (d.end_date) {
        mergeData(
          d.end_date, parseTime(row['End Time']),
        )
      }

      else { throw new TLError('invalid_end_time_without_date') }
    }
 ...
 * @param {string} url
 */
export async function readGoogleAsCSV(
  url, sheets_proxy,
) {
  let rows = []
  ...
  const timeline_config = { events: [], errors: [], warnings: [], eras: [] }
  ...
  rows.forEach((
    ...
    try {
      if (!isEmptyObject(row)) {
        const event = extractEventFromCSVObject(row)
        handleRow(
...
export function makeGoogleCSVURL(url_or_key) {
  ...
  if (url_or_key.startsWith('https://docs.google.com/spreadsheets/')) {
    if (url_or_key.match(/\/pub\?output=csv$/))
      return url_or_key
    const parsed = new URL(url_or_key)
    ...
    const base_path = parsed.pathname.substr(
      0, parsed.pathname.lastIndexOf('/'),
    )
...
const buildGoogleFeedURL = function (
  key, api_version,
) {
  if (api_version == 'v4')
...
async function jsonFromGoogleURL(
  ...
  const timeline_json = await readGoogleAsCSV(
    google_url, options.sheets_proxy,
  )
...
export async function makeConfig(
  ...
  if (key) {
    try {
      ...
      if (e.name == 'NetworkError') { tc.logError(new TLError('network_err')) }

      else if (e.name == 'TLError') { tc.logError(e) }

      else {
        tc.logError(new TLError(
          'unknown_read_err', e.name,
        ))
      }
    ...
    ajax({
      ...
      error(
        ...
        tc = new TimelineConfig()
        if (errorType == 'parsererror') { var error = new TLError('invalid_url_err') }
        else {
          var error = new TLError(
            'unknown_read_err', errorType,
          )
        }
