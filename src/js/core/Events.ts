...
import { mergeData, trace } from './Util'
...
export default class Events {
  ...
  /**
     ...
     */
  addEventListener(
    /* String */ type, /* Function */ fn, /* (optional) Object */ context,
  ) {
    return this.on(
      type, fn, context,
    )
  ...
  removeEventListener(
    ...
    for (let i = 0, events = this._tl_events, len = events[type].length; i < len; i++) {
      if (
  ...
  fire(
    ...
    const listeners = this._tl_events[type].slice()

    for (let i = 0, len = listeners.length; i < len; i++) {
      if (listeners[i].action) {
        listeners[i].action.call(
          listeners[i].context || this, event,
        )
      }
