...
class Timeline {
  ...
  constructor(
    ...
    this.message = new Message(
      this._el.container, { message_class: 'tl-message-full' },
    )
    ...
    if (this.options.is_embed) {
      addClass(
        this._el.container, 'tl-timeline-embed',
      )
    }

    if (this.options.is_full_embed) {
      addClass(
        this._el.container, 'tl-timeline-full-embed',
      )
    }
  ...
  _loadStyles() {
    ...
    if (this.options.theme && (
      ...
      const fragment = `../css/themes/timeline.theme.${this.options.theme.toLowerCase()}.css`
  ...
  determineScriptPath() {
    ...
    if (script_src_url) { // did we get it when this loaded?
      ...
      const script_tag = document.getElementById('timeline-script-tag')
      if (script_tag)
        src = script_tag.src
    }
    ...
    if (this.config.isValid()) {
      try {
        if (document.readyState === 'loading') { // Loading hasn't finished yet
          document.addEventListener(
            'DOMContentLoaded', this._onDataLoaded.bind(this),
          )
        }
        else {
          this._onDataLoaded()
  ...
  _onDataLoaded() {
    ...
    const callback = (
      ...
      if (entries.reduce(
        (
          accum, curr,
        ) => accum || curr.isIntersecting, false,
      ))
        this.updateDisplay()
    }
  ...
  _initLayout() {
    ...
    if (this.options.timenav_position == 'top') {
      this._el.timenav = DOM.create(
        'div', 'tl-timenav', this._el.container,
      )
      this._el.storyslider = DOM.create(
        'div', 'tl-storyslider', this._el.container,
      )
    }
    else {
      this._el.storyslider = DOM.create(
    ...
    this._el.menubar = DOM.create(
      'div', 'tl-menubar', this._el.container,
    )
    ...
    this._timenav = new TimeNav(
      this._el.timenav, this.config, this.options, this.language,
    )
    this._timenav.on(
      'loaded', this._onTimeNavLoaded, this,
    )
    ...
    this._storyslider = new StorySlider(
      this._el.storyslider, this.config, this.options, this.language,
    )
    this._storyslider.on(
      'loaded', this._onStorySliderLoaded, this,
    )
    this._storyslider.init()

    // Create Menu Bar
    this._menubar = new MenuBar(
      this._el.menubar, this._el.container, this.options,
    )
    ...
    // Update Display
    this._updateDisplay(
      this._timenav.options.height, true, 2000,
    )
  }
  ...
  _initEvents() {
    // TimeNav Events
    this._timenav.on(
      'change', this._onTimeNavChange, this,
    )
    this._timenav.on(
      'zoomtoggle', this._onZoomToggle, this,
    )

    // StorySlider Events
    this._storyslider.on(
      'change', this._onSlideChange, this,
    )
    this._storyslider.on(
      'colorchange', this._onColorChange, this,
    )
    this._storyslider.on(
      'nav_next', this._onStorySliderNext, this,
    )
    this._storyslider.on(
      'nav_previous', this._onStorySliderPrevious, this,
    )

    // Menubar Events
    this._menubar.on(
      'zoom_in', this._onZoomIn, this,
    )
    this._menubar.on(
      'zoom_out', this._onZoomOut, this,
    )
    this._menubar.on(
      'back_to_start', this._onBackToStart, this,
    )
  }

  _onColorChange(e) {
    this.fire(
      'color_change', { unique_id: this.current_id }, this,
    )
  }
  ...
  _onChange(e) {
    this.fire(
      'change', { unique_id: this.current_id }, this,
    )
  ...
  _onBackToStart(e) {
    ...
    this.fire(
      'back_to_start', { unique_id: this.current_id }, this,
    )
  }
  ...
  _onZoomIn(e) {
    ...
    this.fire(
      'zoom_in', { zoom_level: this._timenav.options.scale_factor }, this,
    )
  }
  ...
  _onZoomOut(e) {
    ...
    this.fire(
      'zoom_out', { zoom_level: this._timenav.options.scale_factor }, this,
    )
  }
  ...
  _updateDisplay(
    timenav_height, animate, d,
  ) {
    let duration = this.options.duration
    let display_class = this.options.base_class
    let menu_position = 0
    const self = this
    ...
    if (Browser.mobile) {
      ...
      this.options.timenav_height = this._calculateTimeNavHeight(
        timenav_height, this.options.timenav_mobile_height_percentage,
      )
    }
    else {
      // Set TimeNav Height
    ...
    if (animate) {
      ...
      this.animator_storyslider = Animate(
    ...
    this._storyslider.updateDisplay(
      this.options.width, this.options.storyslider_height, animate, this.options.layout,
    )
  ...
  _calculateTimeNavHeight(
    timenav_height, timenav_height_percentage,
  ) {
    let height = 0
  ...
  _validateOptions() {
    ...
    const INTEGER_PROPERTIES = ['timenav_height', 'timenav_height_min', 'marker_height_min', 'marker_width_min', 'marker_padding', 'start_at_slide', 'slide_padding_lr']
  ...
  _onLoaded() {
    if (this._loaded.storyslider && this._loaded.timenav) {
      ...
      if (this.options.hash_bookmark) {
        ...
        else { this._updateHashBookmark(this.current_id) }

        const the_timeline = this
        window.addEventListener(
          'hashchange', () => {
            if (window.location.hash.indexOf('#event-') == 0) {
              the_timeline.goToId(window.location.hash.replace(
                '#event-', '',
              ))
            }
          }, false,
  ...
  _updateHashBookmark(id) {
    if (id) { // TODO: validate the id...
      ...
      window.history.replaceState(
        null, 'Browsing TimelineJS', hash,
      )
      this.fire(
        'hash_updated', { unique_id: this.current_id, hashbookmark: '#' + `event-${id.toString()}` }, this,
      )
    }
  }
  ...
  goToId(id) {
    if (this.current_id != id) {
      this.current_id = id
      this._timenav.goToId(this.current_id)
      this._storyslider.goToId(
        this.current_id, false, true,
      )
      this.fire(
        'change', { unique_id: this.current_id }, this,
      )
  ...
  add(data) {
    ...
    this._storyslider.createSlide(
      d, this.config.title ? n + 1 : n,
    )
    this._storyslider._updateDrawSlides()

    this._timenav.createMarker(
      d, n,
    )
  ...
  remove(n) {
    if (n >= 0 && n < this.config.events.length) {
      ...
      const event = this.config.events.splice(
        n, 1,
      )
      ...
      this.fire(
        'removed', { unique_id: event[0].unique_id },
      )
    }
  }
  ...
  _initGoogleAnalytics() {
    ...
    ga(
      'create', this.options.ga_property_id, 'auto',
    )
    ga(
